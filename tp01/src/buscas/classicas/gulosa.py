...
def gulosa(lab: LabirintoBusca) -> ResultadoBusca:
    t0 = time.perf_counter()
    contador = itertools.count()
    inicio = No(lab.inicio, g=0.0)
    h_inicio = lab.h(lab.inicio)
    fronteira: list = []
    heapq.heappush(fronteira, (h_inicio, next(contador), inicio))
    ...
    while fronteira:
        ...
        if no.estado == lab.objetivo:
            ...
            return ResultadoBusca(
                'Busca Gulosa (Greedy Best-First)', True,
        ...
        for acao, estado, custo in lab.vizinhos(no.estado):
            ...
            if novo_g < melhor_g.get(estado, math.inf):
                ...
                h = lab.h(estado)
                heapq.heappush(fronteira, (h, next(contador), filho))

    return ResultadoBusca(
        'Busca Gulosa (Greedy Best-First)', False, [], [], nos_explorados, nos_expandidos,
        ordem_explorados,
        custo_total=0.0,
        tempo_ms=(time.perf_counter() - t0) * 1000,
        fronteira_max=fronteira_max,
